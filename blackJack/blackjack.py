...
class Player:
    ...
    def initialBet( self ):
        ...
        while True:
            try:
                currentBet = input( "--> " );
                if( currentBet == "" ):
                    currentBet = self.previousBet;
                elif( currentBet == "0" ):
                    currentBet = self.AccountValue;
                currentBet = int( currentBet );
                break;
            except ValueError:
                print( "Please enter a number" );
                
        print( "Press enter if " + "$" + str(currentBet) + " is correct, if not, enter the correct amount here! " );
    ...
    def perfectPlay( self, pCard1Value, pCard2Value, dCard1Value ):

        with open("./blackJack/perfectPlay.txt") as f:
            PlayFile = csv.reader( f, delimiter=" " );
            Play = []
    
            for action in PlayFile:
                Play.append( action );
        ...
        if( pCard1Value == "11" ):
            pCard1Value = "A";
            playerPlayHand = pCard1Value + pCard2Value;
        elif( pCard2Value == "11" ):
            pCard2Value = "A";
            playerPlayHand = pCard2Value + pCard1Value;
        elif( pCard1Value == pCard2Value ):
            playerPlayHand = pCard1Value + pCard2Value;
        else:
            playerPlayHand = str( int( pCard1Value ) + int( pCard2Value ) );
        ...
        for card in Play:
            pHandIndex += 1;
            if( playerPlayHand == card[0] ):
                break;
        pHandIndex -= 1;

        action = Play[pHandIndex][dCardIndex];
        return action;
...
def run():
    ...
    while( player.running == True ):
        ...
        print("Press enter for perfect play!!")
        action = input()
        ...
        while True:
            ...
            if( action.upper() == "HIT" ):
                ...
                if( player.bet <= player.AccountValue / 2 ):
                    ...
                    print("Invalid Option, proceeding to Hit");
                    action = "HIT"
                    continue;
                ...
                pCard1Value = str( deck.evaluateHandTotal( list( player.hand[0] ) ) );
                pCard2Value = str( deck.evaluateHandTotal( list( player.hand[1] ) ) );
                dCard1Value = str( deck.evaluateHandTotal( list( dCard1 ) ) );
                action = player.perfectPlay( pCard1Value, pCard2Value,dCard1Value );

                print( "Perfect play dictates you "+ action );
                sleep(2);
                print();
                continue;
